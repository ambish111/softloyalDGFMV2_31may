...
//     print_r($customer_data);die;
        $zid_status = 'ready';//$customer_data['zid_status'];
        ...
        if($customer_data['zid_authorization'] != NULL){
            $this->athentication = $customer_data['zid_authorization'];
            $url="https://fm.diggipacks.com/zid_v2/getOrder/".$customer_data['super_id'].'/Y';
        }else{
            $url="https://fm.diggipacks.com/zid/getOrder/$cust_id";
        }
        
//        echo $url ;die;
//        echo $this->athentication;die;
...
//echo '<pre>xxxx'; print_r($result);
                    ...
                    if ($check_booking_id) {
                        ...
                        // echo '<pre>yyy'; print_r($nData);
                         $orderdata = $nData['order'];
                           echo'<br> else'. $this->CreateOrderrequest($orderdata,$uniqueid,$url); //exit;
                    }
                }
            }
        } else {
            ...
            foreach ($result['orders'] as $orders) {
                ...
                if ($check_booking_id) {
                 ...
                 // echo '<pre>yyy'; print_r($nData);
                  $orderdata = $nData['order'];
                    echo'<br> else '.$booking_id.':'. $this->CreateOrderrequest($orderdata,$uniqueid,$url); //exit;
                    }
    ...
    private function CreateOrderrequest($data=array(),$cust_id=null,$url=null) {
...
if (flock($filehandle, LOCK_EX | LOCK_NB)) {
    ...
    $myfile = fopen("/var/www/html/diggipack_new/zid_cron_logs.txt", "a") or die("Unable to open file!");
