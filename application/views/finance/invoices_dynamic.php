...
<script src="<?=base_url();?>assets/js/angular/finance.app.js?v=<?=time();?>"></script>
...
<!-- Page container dynamicinvoiceReport(1,1);-->
<div class="page-container" ng-controller="CTR_allinvoicesView" ng-init="getallseller('Dynamic');">
    ...
    <table class="table table-bordered table-hover" style="width: 100%;">
      ...
      <tbody >
        <tr style="width: 80%;">
          ...
          <td><button  class="btn btn-danger" ng-click="dynamicinvoiceReport(1,1);" ><?=lang('lang_Get_Details');?></button></td>
           <td>
          <a class="btn btn-danger" ng-click = "run_shell_dynamic();" target="_blank" ><?=lang('lang_Sync');?></a>
      ...
      <table class="table table-striped table-hover table-bordered dataTable bg-* display nowrap" style="width:100%; overflow:scroll;">
        <thead>
          ...
          <th><?=lang('lang_Sr_No');?>.</th>
            ...
            <!--  <th>Pickup Charge</th>
             ...
             <th>Dispatch Charge</th>
             <th>Inbound Charge</th>
             <th> Outbound Charge</th>
             <th>Inventory Charge</th>
             <th>Portal Charge</th>
             <th> Sku Barcode Print</th>
        ...
        <tr ng-if='showlistData!=0' ng-repeat="data in showlistData">
         ...
         <!--  <td> {{data. pickup_charge}}</td>
          <td>SAR {{data.special_packing_seller -- data.special_packing_warehouse}}</td>
          <td>SAR {{data.return_charge}}</td>
          <td>SAR {{data.shipping_charge}}</td>
          <td>SAR {{data.onhold_charges}}</td>
          <td>SAR {{data.packing_charge}}</td>
          <td>SAR {{data.packing_charge}}</td>
          <td>SAR {{data.picking_charge}}</td>
          <td>SAR {{data.dispatch_charge}}</td>
          <td>SAR {{data.inbound_charge}}</td>
          <td>SAR {{data.outbound_charge}}</td>
          <td> SAR {{data.inventory_charge}}</td>
          <td> SAR {{data.portal_charge}}<br>
          <td>SAR {{data.sku_barcode_print}}</td>
          <td> SAR {{data.storage_charge}}</td>
          <td> SAR {{data.box_charge}}</td>
          <td>SAR {{data.total_vat}} 
          </td> -->
              ...
              <li class="dropdown"> <a href="#" class="dropdown-toggle" data-toggle="dropdown"> <i class="icon-menu9"></i> </a>
                <ul class="dropdown-menu dropdown-menu-right">
                  ...
                  <li ><a  href="<?=base_url();?>viewinvoice_dynamic/{{data.invoice_no}}" target="_blank"><i class="icon-eye" ></i>   <?=lang('lang_View_Invoice');?> </a></li>
                   <li ><a  href="<?=base_url();?>viewinvoice_dynamic_new/{{data.invoice_no}}" target="_blank"><i class="icon-eye" ></i>   <?=lang('lang_View_Invoice');?> New </a></li>
<li ><a  href="<?=base_url();?>Bulkdownload_in/getExport/{{data.invoice_no}}" target="_blank"><i class="icon-eye" ></i>   Download </a></li>
<li ><a  href="<?=base_url();?>Bulkdownload_in/getExport_new/{{data.invoice_no}}" target="_blank"><i class="icon-eye" ></i>   Download New</a></li>
                  <li>
