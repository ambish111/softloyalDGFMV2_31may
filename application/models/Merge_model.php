...
class Merge_model extends CI_Model {
    ...
    public function Getallstocklocationdata_viewpage($seller_id = null,$stockLoction) {
        ...
        $this->db->where('`stock_location` NOT IN (SELECT `stock_location` FROM `item_inventory` where super_id= '.$this->session->userdata('user_details')['super_id'].'  and stock_location!="" and seller_id='.$seller_id.' )', NULL, FALSE);
        $this->db->where('seller_id', $seller_id);
        $this->db->where('stock_location', $stockLoction);
        $query = $this->db->select('seller_id,stock_location,0 as quantity')->get('stockLocation');
        return $query->result_array();
    ...
    {
      $this->db->where('item_inventory.super_id', $this->session->userdata('user_details')['super_id']);
      return $this->db->update_batch('item_inventory',$data,'id');
    }
    ...
    {
        $this->db->insert_batch('inventory_activity',$data);
    }
