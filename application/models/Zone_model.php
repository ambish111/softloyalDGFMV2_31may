...
class Zone_model extends CI_Model
{
	...
	public function getCityColumnByCname($companyName = NULL)
	{
		...
		$cityColumnArray = array(
			...
			'j&t eg' => 'jt_eg_city',
			'weenkapp' => 'weenkapp_city_id',
			'roz express' => 'rozx_city',
			'sprint' => 'sprint_city',
			'Torod' => 'torod_city',
			'shipadelivery v2' => 'shipsa_city',
			'kwickbox' => 'kwickbox_city',
			'nashmi' => 'shipox_city_name',
			'dal' => 'dal_city',
			'ajex' => 'ajex_city',
			'coldt' => 'shipox_city_name',
			'td logistics'=>'shipsy_city',
	...
	public function get_old_city($cc_id=NULL){
		$this->db->where('super_id', $this->session->userdata('user_details')['super_id']);
		$this->db->select('city_id');
		$this->db->where('cc_id',$cc_id);
		$this->db->where('deleted','N');
		// $this->db->order_by('city_id');
		// $this->db->group_by('city');
		$query=$this->db->get('zone_list_fm');
		// echo $this->db->last_query();
		if($query->num_rows()>0){
	...
	public function deleteCourierZoneById($id = null)
	{
		$this->db->from('zone_list_fm');
		$this->db->where('id', $id);
		$this->db->update('zone_list_fm', array('deleted' => 'Y'));
		// echo $this->db->last_query(); die;
	}
}
