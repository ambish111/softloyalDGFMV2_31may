...
class Backorder_model extends CI_Model {
    ...
    public function getcheckSku($sku=null,$seller_id=null,$super_id=null,$wh_id=null)
    {
        // if ($this->session->userdata('user_details')['user_type'] != 1) {
        //     
        // }
        $this->db->where('item_inventory.wh_id',$wh_id);
        $this->db->where('item_inventory.seller_id', $seller_id);
        $this->db->where('items_m.super_id', $super_id);
        $this->db->where('item_inventory.super_id', $super_id);
        $this->db->select('SUM(item_inventory.quantity) as tqty,items_m.sku,items_m.id');
        $this->db->from('items_m');
        $this->db->join('item_inventory','items_m.id=item_inventory.item_sku','LEFT');
   ...
   public function allOgOrders($super_id=null,$wh_id=null) {
