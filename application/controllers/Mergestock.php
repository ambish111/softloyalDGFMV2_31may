...
class Mergestock extends MY_Controller {
    ...
    public function index() {
        ...
        $quantity = $this->Merge_model->count_all();
        $sellers = $this->Seller_model->find1();
        $StorageType = $this->Merge_model->GetstorageTypes();
        ...
        $this->load->view('merge/search', $bulk);
    }
    ...
    public function filter() {
        ...
        $items = $this->Merge_model->filter($quantity, $sku, $seller, $to, $from, $exact, $page_no, $shelve_no, $storage_id, $_POST);
    ...
    public function GetallStockLocation_new() {
        ...
        $result = $this->Merge_model->check($param);
        if (count($result) > 0) {
            echo json_encode($result);
            exit;
        } elseif (count($result) == 0) {
            $result = $this->Merge_model->Getallstocklocationdata_viewpage($seller_id, $newstockLocation);
            if (count($result) > 0) {
                echo json_encode($result);
                exit;
            } else {
    ...
    public function GetMergeReadyStock() {
        ...
        if (!empty($mainUpArr)) {
              ...
              $WB_Confing = webhook_settingsTable_in($filterArr['seller']);
                if ($WB_Confing['subscribe'] == 'Y') {
                    $sku_name = getalldataitemtables($filterArr['item_sku'], 'sku');
                    $wbh_array=array();
                        $wb_request = array(
                        'datetime' => date('Y-m-d H:i:s'),
                        "sku" => $filterArr['item_sku'],
                        "cust_id" => $filterArr['seller'],
                        'sku_name' => trim($sku_name),
                        "order_from" => "Merge Inventory",
            ...
            if (!empty($updateFirstStock)) {
                $this->Merge_model->updatingData($updateFirstStock);
                $this->Merge_model->inserthistoryData($inventoryhistory);
            }
            if (!empty($insertData)) {
                $this->Merge_model->insertstockData($insertData);
