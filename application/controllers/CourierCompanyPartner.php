...
class CourierCompanyPartner extends MY_Controller  { 
	function __construct() {
        ...
        $this->load->model('CourierCompanyPartner_model');
	...
	public function viewFpCompany(){
		$this->load->view('courierCompanyPartner/view_company_new');               
	}
    ...
    public function getccdetail(){
        ...
        $cc_id = $postArr['id']; 
        $result = $this->CourierCompanyPartner_model->getFPCompanyDetails($cc_id);
        ...
        {
            $postArr = json_decode(file_get_contents('php://input'), true);
            $return = $this->CourierCompanyPartner_model->all_new();
...
//            if($fp_status =='Y'){
//                $postArr = json_decode(file_get_contents('php://input'), true);
//                $return = $this->CourierCompanyPartner_model->all();
//                
//                $cc_array = array();
//                if(!empty($return)){
//
//                    $i=0;
//                    foreach($return as $cc_data){
//                        $cc_array[$i] = $cc_data;
//                        $fp_status = $this->CourierCompanyPartner_model->getFPstatus($cc_data->cc_id);
//                        //print "<pre>"; print_r($fp_status);die;
//                        if(!empty($fp_status)){
//                            $cc_array[$i]->fp_status  = 'Y';
//                        }else{
//                            $cc_array[$i]->fp_status  = 'N';
//                        }
//                        $cc_array[$i]->type  = $fp_status['type'];
//
//                        $i++;
//                    }
//
//                }
//               // print "<pre>"; print_r($cc_array);die;
...
//    }
    ...
    public function GetUpdateActiveStatus()
    {
        ...
        if($_POST['status'] == 'Y'){
            $return = $this->CourierCompanyPartner_model->all($data_w);
           // print "<pre>"; print_r($return);die;
            if(!empty($return)){

                $UpdateArr=array(
                    'user_name'=>$return[0]->user_name,
                    'company_url'=>$return[0]->company_url,
                    'password'=>$return[0]->password,
                    'courier_account_no'=>$return[0]->courier_account_no,
                    'courier_pin_no'=>$return[0]->courier_pin_no,
                    'start_awb_sequence'=>$return[0]->start_awb_sequence,
                    'end_awb_sequence'=>$return[0]->end_awb_sequence,
                    'auth_token'=>$return[0]->auth_token,
                    'api_url'=>$return[0]->api_url,
                    'user_name_t'=>$return[0]->user_name_t,        
                    'password_t'=>$return[0]->password_t,
                    'courier_account_no_t'=>$return[0]->courier_account_no_t,
                    'courier_pin_no_t'=>$return[0]->courier_pin_no_t,
                    'start_awb_sequence_t'=>$return[0]->start_awb_sequence_t,
                    'end_awb_sequence_t'=>$return[0]->end_awb_sequence_t,
                    'auth_token_t'=>$return[0]->auth_token_t,
                    'api_url_t'=>$return[0]->api_url_t,
                    'customer_care'=>$return[0]->customer_care,
                    'account_entity_code'=>$return[0]->account_entity_code,
                    'account_entity_code_t'=>$return[0]->account_entity_code_t,
                    'account_country_code'=>$return[0]->account_country_code,
                    'account_country_code_t'=>$return[0]->account_country_code_t,
                    'service_code'=>$return[0]->service_code,
                    'service_code_t'=>$return[0]->service_code_t,
                    'super_id'=> $this->session->userdata('user_details')['super_id'],
                    'fp_flag'=>'Y',
                    'image'=>$return[0]->image,
                    'company'=>$return[0]->company,
                    'cc_id'=>$return[0]->cc_id
               );

              // print "<pre>"; print_r($UpdateArr);die;
              $this->CourierCompanyPartner_model->InsertDeliveryCOmpany($UpdateArr);
            ...
            $cc_id = $_POST['cc_id'];
            // echo $cc_id;die;
            $this->CourierCompanyPartner_model->UnsubscribePartnerCompany($cc_id);
        }
    ...
    public function GetUpdateLIveStatus()
    {
         ...
         $data_w=array('id'=>$_POST['id'],'cc_id'=>$_POST['cc_id']);
         $return=$this->CourierCompanyPartner_model->GetUpdateDeliveryCOmpany($data,$data_w);
         echo json_encode($return);
    }
            ...
            foreach ($shipmentLoopArray as $key => $slipNo) 
            {
             ...
             // echo "<pre>"; print_r($counrierArr); die; 
                ...
                if(!empty($ShipArr))
                {
                    ...
                    // if($super_id == 175)
                    // {             
                    //     $sellername = $ShipArr['sender_name'].' - DIGGIPACKS FULLFILLMENT';                      
                    ...
                    $seller_label_info = GetallCutomerBysellerId($ShipArr['cust_id'],'label_info_from');     

                    echo "seller_label_info=".$seller_label_info;  die;
                   ...
                   if($ccRetrundata['status']==200)
                    {
                        $Update_data = $this->Ccompany_model->Update_Shipment_Status($slipNo, $ccRetrundata['client_awb'], $CURRENT_TIME, $CURRENT_DATE, $company, $comment, $ccRetrundata['label'],$c_id,$ccRetrundata['barq_order_id'], $api_url,$ccRetrundata['bosta_label_id'] );
                    ...
                    elseif ($company == 'SLS')
                    {
                       ...
                       else
                       {
                        ...
                        if (isset($responseArray['ShipmentResponse']['Response']['ResponseStatus']) && $responseArray['ShipmentResponse']['Response']['ResponseStatus']['Code'] == 1) {
                            ...
                            //file_put_contents("assets/all_labels/$slipNo.pdf", $response_label);
                            $dd= file_put_contents("/var/www/html/diggipack_new/demofulfillment/assets/all_labels/$slipNo.pdf", $response_label);
