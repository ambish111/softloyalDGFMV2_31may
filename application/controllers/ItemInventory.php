...
class ItemInventory extends MY_Controller {
    ...
    public function damage_list_view() {
        ...
        $this->load->view('return_manifest/damage_list_view.php', $bulk);
    }
 public function update_expire($id=null) {
       $view['table_id']=$id;
       $edit_result=$this->ItemInventory_model->edit_view($id);
       $view['edit_result']=$edit_result[0];
      // print_r($view); die;
       $this->load->view('ItemI/update_expire',$view);
    }
    public function GetUpdateInventoryExpire($id=null){
        if(!empty($id))
        {
            ...
            $this->ItemInventory_model->UpdateExpireDate($updateArray,$id);
    ...
    public function ViewStock() {
        ...
        $this->load->view('ItemI/ViewStock', $bulk);
    }
    ...
    public function add_view($page_type = null) {
    ...
    public function add() {
        ...
        $_POST['sku'] = getallitemskubyid($this->input->post('sku'));
        ...
        $WB_Confing = webhook_settingsTable_in($this->input->post('seller'));
        if ($WB_Confing['subscribe'] == 'Y') {
            $wbh_array = array();
            $wb_request = array(
                'datetime' => date('Y-m-d H:i:s'),
                "sku" => $this->input->post('sku'),
                "cust_id" => $this->input->post('seller'),
                'sku_name'=>$sku_name,
                "order_from" => "Single Inventory",
        ...
        if (!empty($token)) {   //Qty update in Zid section 
            ...
            $reszid = update_zid_product($quantity, $pid, $token, $storeID, $seller_id, $sku_name);
            //=========================================//
        }

        if (empty($page_type) && ($seller_id == '214' || $seller_id == '31')) {
            //echo "ssss"; die;




            $this->Manifest_model->getupdateTempstock($chargeQty, $seller_id, $sku_name);
            $temp_stock = $this->Manifest_model->GetTempStockData($sku_name, $seller_id);
            //$sall_update_qty=$temp_stock['qty']+$skuqtyval['totalqty'];
            salla_provider_qty_update($temp_stock['qty'], $uniqueid, $sku_name, $seller_id, 'single invenotry');
        }
        if (!empty($sallatoken)) {
    ...
    public function filter_viewStock() {


        $postData = json_decode(file_get_contents('php://input'), true);

        $items = $this->ItemInventory_model->filter_viewStock($postData);
        $ItemArray = $items['result'];
        ...
        foreach ($items['result'] as $rdata) {
            ...
            // $ItemArray[$kk]['item_type'] = getalldataitemtables($rdata['id'], 'type');
            // $ItemArray[$kk]['storage_id'] = Getallstoragetablefield(getalldataitemtables($rdata['item_sku'], 'storage_id'), 'storage_type');
            //$ItemArray[$kk]['wh_name'] = Getwarehouse_categoryfield($rdata['wh_id'], 'name');
    ...
    public function GetallStockLocation_new() {
        ...
        $seller_id = $_POST['seller_id'];
        //$skuID = $_POST['sku'];
        $skuID = getallitemskubyid($_POST['search_sku']); //$_POST['sku'];
        $sku_size = getalldataitemtables($skuID, 'sku_size');
        if ($sku_size >= $quantity)
            $locationLimit = 1;
        else {
        ...
        // echo 'xxxxx-'. $locationLimit;  exit;
        if (empty($locationLimit) || $locationLimit == INF) {
            $locationLimit = 1;
        }
        $data = array();
        ...
        $pending_loation = $locationLimit - $totallocation;
        $returnarray['CountLocation'] = $locationLimit;
        $returnarray['CountLocation_pending'] = $pending_loation;
        echo json_encode($returnarray);
    }
...
//	print_r($dataArray); die;
    ...
    function exportexcelinventory_viewStock() {
        ini_set('memory_limit', '5000000M');
        ini_set('max_execution_time', 1200);
        $_POST = json_decode(file_get_contents('php://input'), true);
        // print_r($_POST); die;
        $exportlimit = $_POST['exportlimit'];
        $shipmentsexcel = $this->ItemInventory_model->exportexcelinventory_viewStock($_POST);
        ...
        foreach ($dataArray as $data) {
            ...
            $DatafileArray[$i]['seller_name'] = $data['seller_name'];
            $DatafileArray[$i]['item_type'] = $data['type'];
            $DatafileArray[$i]['sku'] = $data['sku'];
            $DatafileArray[$i]['quantity'] = '0';
...
//	print_r($dataArray); die;
    ...
    public function GetUpdateMissingOrDamgeQty() {
        ...
        $seller_id = $_POST['sid'];
        $uniqueid = GetallCutomerBysellerId($seller_id, 'uniqueid');
        $sku_name = getalldataitemtables($_POST['item_sku'], 'sku');
        ...
        if ($seller_id == '214' || $seller_id == '31') {
            ...
            $this->Manifest_model->getupdateTempstock_deducted($_POST['item_sku'], $seller_id, $sku_name);
            $temp_stock = $this->Manifest_model->GetTempStockData($sku_name, $seller_id);
            //$sall_update_qty=$temp_stock['qty']+$skuqtyval['totalqty'];
            salla_provider_qty_update($temp_stock['qty'], $uniqueid, $sku_name, $seller_id, 'Qty Updated By ' . getUserNameById($this->session->userdata('user_details')['user_id']));
        }
    ...
    public function Gettop10productshow() {
        ...
        $items = $this->ItemInventory_model->Gettopproductdatashow($quantity, $sku, $seller, $to, $from, $exact, $page_no, $_POST);
        $ItemArray = $items['result'];
        //print_r($ItemArray);
        $kk = 0;
        //echo '<pre>';
    ...
    public function GetCustomerNamesData() {
        $return = Getallsellerdata_new();
        echo json_encode($return);
    }

    public function shelve_report_new() {

        $quantity = $this->ItemInventory_model->count_all_new();
        $sellers = $this->Seller_model->find1();
        $StorageType = $this->ItemInventory_model->GetstorageTypes_new();
        // $data['items'] = $this->ItemInventory_model->all();
        $condition = null;
        if ($this->session->flashdata('condition')) {
            $condition = $this->session->flashdata('condition');
        ...
        $this->load->view('ItemI/shelve_report_new', $bulk);
    }

    public function filter_shelve_new() {
        ...
        $items = $this->ItemInventory_model->filter_shelve_new($quantity, $sku, $seller, $to, $from, $exact, $page_no, $shelve_no, $storage_id, $_POST);
        $ItemArray = $items['result'];
        //print_r($ItemArray);
        $kk = 0;
        $jj = 0;
    ...
    public function GetSearchItemlist() {
        $PostData = json_decode(file_get_contents('php://input'), true);
        $return = $this->ItemInventory_model->GetSearchItemlist($PostData['search_sku']);
        foreach ($return as $val) {
            $return_sku[] = $val['sku'];
        }
        echo json_encode($return_sku);
    }
