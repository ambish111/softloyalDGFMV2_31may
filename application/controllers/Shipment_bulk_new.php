...
class Shipment_bulk_new extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->load->helper('stock_new');
        $this->load->helper('zid');
        $this->load->helper('utility');
        $this->load->model('Shipment_bulk_new_model');
    }
    ...
    public function bulk_create() {
        ...
        if (menuIdExitsInPrivilageArray(254) == 'N') {
             redirect(base_url() . 'notfound');
             die;
        }
        $this->load->view('ShipmentM/bulk_order_create_new');
    }
    ...
    public function checkorderValid() {
        ...
        $DataArray = $postData;
        $shipments = $this->Shipment_bulk_new_model->checkvalid($DataArray);
        $valid = array();
        $invalid = array();
...
//        error_reporting(E_ALL);
        ...
        if (!flock($file, LOCK_EX | LOCK_NB)) {
            ...
            if (!empty($shipArr)) {
                ...
                $shipments = $this->Shipment_bulk_new_model->checkvalid($shipArr);
                // print_r($shipments);
                if (!empty($shipments)) {
                    $wbh_array = array();
                    $wbh_array_in = array();
                    ...
                    foreach ($shipments as $key => $data) {
                        if ($data['code'] == 'OG' && $data['delivered'] == 11) {
                            if ($data['origin'] > 0 && $data['destination'] > 0 && $data['wh_id'] > 0 && $data['pieces'] > 0 && $data['backorder'] == 0 && $data['code'] == 'OG' && $data['delivered'] == 11 && !empty($data['reciever_name']) && !empty($data['reciever_address'])) {
                                ...
                                if (!empty($r_city)) {
                                    ...
                                    if (!empty($data['skuData'])) {
                                        ...
                                        foreach ($data['skuData'] as $new_key => $skuDetails) {
                                           ...
                                           // print_r($sku_main_arr);
                                            $skuDetails['sku_id'] = $sku_main_arr['sku_id'];
                                            $skuDetails['i_weight'] = $sku_main_arr['weight'];
                                            $skuDetails['expire_block'] = $sku_main_arr['expire_block'];
                                            if ($skuDetails['sku_id'] > 0) {
                                                $stock_check = CheckStockBackorder_ordergen_bulk($data, $skuDetails);
                                                if ($stock_check['succ'] == 1) {
                                                    $ReturnstockArray[] = $stock_check['stArray'];
                                                    $weightcount = $skuDetails['i_weight'];
                                                    ...
                                                    if (!empty($salatoken)) {
                                                        if ($custmoerID != '214') {
                                                            ...
                                                            // echo "<pre>"; print_r($sallaReqArr);
                                                             update_salla_qty_product($quantity, $pid, $sallatoken, $custmoerID);
                                                        }
                                                    }
                                        ...
                                        if (empty($stockarray)) {
                                            ...
                                            UpdateStockorderGen_bulk($ReturnstockArray, $data['weight'], $data['slip_no']);
                                        ...
                                        $errorReturnArray = array("slip_no" => $data['slip_no'], "sku" => null, 'error_type' => 'invalid_sku');
                                        array_push($stockarray, $errorReturnArray);
                                        array_push($stockarray_new, $errorReturnArray);
                                        // array_push($error_array, $errorReturnArray);
                                    }
