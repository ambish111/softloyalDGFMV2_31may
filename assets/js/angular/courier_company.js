var app = angular.module('CourierAppPage', [])
        .controller('CourierComapnyCRL', function ($scope, $http, $window, $location) {
            ...
            $scope.scan = {};
            $scope.newarray = [];
            $scope.EditDataArr.wharehouse_flag = 'N';

            $scope.GetAllCompanyList = function ()
            {
                // alert("ssssss");
                $http({
                    url: URLBASE + "CourierCompany/GetshowcompanyList",
                    method: "POST",
            ...
            $scope.BulkForwardCompanyNew = function ()
            {
                ...
                $http({
                    url: URLBASE + "Shipment/runshellForward",
                    method: "POST",
            ...
            $scope.company_id = {};
            $scope.showOpenPackageBox = function(cc_id){
                $scope.company_id.cc_id = cc_id;
            };
            ...
            $scope.webhookcourier = function (page_no, reset) {
                ...
                $http({
                    url: URLBASE + "CourierCompany/WebhookCourier",
                    method: "POST",
                    ...
                    if (response.data.length > 0) {
                        angular.forEach(response.data, function (value) {
                            // console.log(value)
                            $scope.webhookListArr.push(value);
            ...
            $scope.updatewebhookurl = function () {
                // alert($scope.EditDataArr.cc_name)           
                if ($scope.EditDataArr.cc_name == undefined || $scope.EditDataArr.WEBHOOKURL == undefined) {
                    ...
                    $http({
                        url: URLBASE + "CourierCompany/updatewebhookurl",
                        method: "POST",
                        data: $scope.EditDataArr,
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                        ...
                        $scope.Companywehbookupdate = response.data;
            ...
            $scope.Update3plInfo = function ()
            {
                ...
                $http({
                    url: URLBASE + "ShipmentUpdate/ShipmentUpdate_info",
                    method: "POST",
                    data: $scope.userselected,
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'}
        ...
        .controller('forward_shipment_view', function ($scope, $http) {
            ...
            $scope.GetCompanylistDropTorod = function ()
            {
                $http({
                    url: URLBASE + "CourierCompany/GetCompanylistDropTorod",
                    method: "POST",
                    data: $scope.UpdateStatusArr,
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'}
            ...
            $scope.torod = '';
            $scope.loadMoreTorod = function (page_no, reset, torod)
            {
                disableScreenTorod(1);
                $scope.loadershow = true;
                $scope.filterData.forwarded_type = 0;
                // console.log(page_no);
                // console.log($scope.selectedData);    
            ...
            function disableScreenTorod(val) {
                if (val == 1)
                { 
                    //   var div = document.createElement("div");
                    //   div.className += "overlay";
                    //   document.body.appendChild(div);
                    $( "#torodload" ).addClass( "overlay1" );
                } else
                      $("#torodload").removeClass("overlay1");
    
           
             }
            $scope.Getforwared3plcompanyTorod = function ()
            {
                ...
                if ($scope.userselected.cc_id)
                {
                    ...
                    $http({
                        url: URLBASE + "TorodForward/BulkForwardCompanyTorod",
                        method: "POST",
                        data: {'slip_arr': $scope.Items, 'otherArr': $scope.userselected},
                        headers: {'Content-Type': 'application/x-www-form-urlencoded'}
            ...
            $scope.warehouse = [];
            $scope.GetWarehouseTorod = function ()
            {
                $http({
                    url: URLBASE + "Shipment/Showlist_address_wherehouse",
                    method: "POST",
                    data: '',
