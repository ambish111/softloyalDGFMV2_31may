var app = angular.module('fulfillPackingEAN', ['betsol.timeCounter'])
        ...
        .controller('scanShipment', function ($scope, $http, $interval, $window) {
            ...
            $scope.mainAWBBtn = false;
          
            $scope.GetremoveBtn = false;
            $scope.disblaeCps_id = false;
                    ...
                    $scope.loadershow = false;
                        ...
                        angular.forEach(response.data.result, function (value) {
                            ...
                            angular.forEach(JSON.parse(value.sku), function (value1) {
                                ...
                                // $scope.shipData_cps.push({'slip_no': value.slip_no, 'sku': value1.sku, 'piece': value1.piece, 'ean_no': value1.ean_no, 'scaned': 0, 'extra': 0, 'cps_id': ''});
                                //note 
                                //ean_no=sku
                                //sku=ean_no


                                //$scope.shipData_cps.push({'slip_no': value.slip_no, 'sku': value1.ean_no, 'ean_no': value1.sku, "serial_details": []});
                                $scope.shipData.push({'slip_no': value.slip_no, 'sku': value1.ean_no, 'ean_no': value1.sku, 'piece': value1.piece, 'scaned': 0, 'extra': 0, 'print_url': value.print_url, 'frwd_company_id': value.frwd_company_id, 'frwd_company_awb': value.frwd_company_awb, 'invoice_label': value.invoice_label,'booking_id':value.booking_id,"serial_details": []});
                ...
                if ($scope.arrayIndex == -1)
                {
                    ...
                    $scope.loadershow = false;
                        ...
                        angular.forEach(response.data.result, function (value) {
                                $scope.awbArray.push(value);
                                angular.forEach(JSON.parse(value.sku), function (value1) {
                                $scope.shipData.push({'slip_no': value.frwd_company_awb, 'sku': value1.ean_no, 'ean_no': value1.sku, 'piece': value1.piece, 'scaned': 0, 'extra': 0, 'print_url': value.print_url, 'frwd_company_id': value.frwd_company_id, 'frwd_company_awb': value.frwd_company_awb, 'invoice_label': value.invoice_label,'booking_id':value.booking_id,"serial_details": [],'slip_new':value.slip_no});
            ...
            {
                $scope.scan.slip_no_new = $scope.scan.slip_no;
            ...
            $scope.scanCPSnumber = function ()
            {
                ...
                $scope.arrayIndex_cps = $scope.shipData.findIndex(record => (record.slip_no.toUpperCase() === $scope.scan.slip_no_new.toUpperCase() && record.sku.toUpperCase().replace(/\s/g, '') === $scope.scan.sku_new.toUpperCase().replace(/\s/g, '')));
            ...
            $scope.finishScanFwd = function ()
            {
                ...
                if ($scope.completeArray.length > 0)
                {
                    ...
                    if (isconfirm)
                    {
                        ...
                        $scope.loadershow = true;
                        $http({
                            url: "PickUp_en/packFinishCPSFWD",
                            method: "POST",
                            data: {
                                shipData: $scope.completeArray,
                                exportData: $scope.shipData,
