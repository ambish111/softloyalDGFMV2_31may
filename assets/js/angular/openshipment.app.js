var app = angular.module('OpenshipAPP', [])
 ...
 .controller('oepnship_CTLR', function ($scope, $http, $interval, $window) {
            ...
            $scope.scan_awb_cancel = function () {
                //$('#scan_awb').focus();
                console.log($scope.scan);
                $scope.scan.awbArray = removeDumplicateValue($scope.scan.slip_no.split("\n"));
                console.log($scope.scan.awbArray);
                $scope.scan.slip_no = $scope.scan.awbArray.join('\n');
                $scope.validateOrder_cancel();
            }
            ...
            $scope.dispatchOrder = function ()
            {
                ...
                $http({
                    url: "OpenShipment/openorderStatusProcess",
                    method: "POST",
                    data: $scope.scan,
            ...
            $scope.cancelorderProcess = function ()
            {
               disableScreen(1);
                $scope.loadershow = true;
                $scope.scan.type = $scope.type;
                $scope.scan.comments = $scope.comments;
                $http({
                    url: "OpenShipment/cancelorderProcess",
                    method: "POST",
                    data: $scope.scan,
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'}
            ...
            $scope.validateOrder = function () {
                ...
                //console.log($scope.scan);
                $http({
                    url: "OpenShipment/validateDispatch",
                    method: "POST",
             ...
             $scope.validateOrder_cancel = function () {
                $scope.invalid = [];
                $scope.warning = null;
                $scope.Message = null;
                // alert("ss");
                ...
                //console.log($scope.scan);
                $http({
                    url: "OpenShipment/validateDispatch_cancel",
                    method: "POST",
                    data: $scope.scan.awbArray,
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'}
