var app = angular.module('AppOrderGen', [])
        ...
        .controller('OrderGenCRTL', function ($scope, $http, $window, Excel, $timeout, $location) {
            ...
            $scope.loadMore = function (page_no, reset, back_order)
            {
                ...
                $http({
                    url: SITEAPP_PATH + "Shipment_og/filter_orderGen",
                    method: "POST",
                    data: $scope.filterData,
                    ...
                    if (response.data.result.length > 0) {
                        angular.forEach(response.data.result, function (value) {
                            ...
                            if ($scope.dataIndex != -1)
                            {
                                ...
                                $scope.shipData[$scope.dataIndex].skuData.push({'sku': value.sku, 'piece': value.piece, 'cod': value.cod, 'wh_id': value.whid, 'sku_id': value.sku_id, 'd_id': value.d_id, 'free_sku': value.free_sku, 'back_reason': value.back_reason});   //scope.shipData[$scope.dataIndex].piece=parseInt($scope.shipData[$scope.dataIndex].piece)+parseInt(value.piece);    
                            } else
                            {
                                ...
                                $scope.shipData[$scope.dataIndex].skuData.push({'sku': value.sku, 'piece': value.piece, 'cod': value.cod, 'wh_id': value.whid, 'sku_id': value.sku_id, 'd_id': value.d_id, 'free_sku': value.free_sku, 'back_reason': value.back_reason});
                            }

                            $scope.dataIndex1 = $scope.shipDataOLD.findIndex(record => record.slip_no === value.slip_no);
                            if ($scope.dataIndex1 != -1)
                            {
                                $scope.shipDataOLD[$scope.dataIndex1].skuData1.push({'sku': value.sku, 'piece': value.piece, 'cod': value.cod, 'wh_id': value.whid, 'sku_id': value.sku_id, 'd_id': value.d_id, 'free_sku': value.free_sku, 'back_reason': value.back_reason});   //scope.shipData[$scope.dataIndex].piece=parseInt($scope.shipData[$scope.dataIndex].piece)+parseInt(value.piece);    
                            ...
                            } else
                            {
                                ...
                                $scope.shipDataOLD[$scope.dataIndex1].skuData1.push({'sku': value.sku, 'piece': value.piece, 'cod': value.cod, 'wh_id': value.whid, 'sku_id': value.sku_id, 'd_id': value.d_id, 'free_sku': value.free_sku, 'back_reason': value.back_reason});
            ...
            $scope.selectAll = function (val) {
                ...
                angular.forEach($scope.shipData, function (data, key) {
                    ...
                    } else
                    {
                        ...
                        //console.log($scope.selectedAll);
                        if (data.reciever_name != '' && data.reciever_phone != '' && data.origin_valid != 0 && data.destination_valid != 0)
                        {
                           // console.log(data.slip_no)
                        ...
                        else
                        {
                           ...
                           //  console.log(data.slip_no)
                            //console.log($scope.Items);
                            $scope.dataIndex = $scope.Items.findIndex(record => record === data.slip_no);
                            if($scope.dataIndex!==-1)
                            {
            ...
            $scope.CreateOrderCheck = function ()
            {
                ...
                } else
                {
                    ...
                    $http({
                        url: SITEAPP_PATH + "Shipment_og/CreateGenratedOrderCheck",
                        method: "POST",
                        data: {
                ...
                else
                {
                       disableScreen(0);
                        $scope.loadershow = false;
                    alert("Order Create Max limit is 200");
                }
            ...
            $scope.runshell = function ()
            {
                ...
                $scope.loadershow = true;
                $http({
                    url: SITEAPP_PATH +"Shipment_og/runshellbackorder",
                    method: "POST",
                    ...
                    $scope.loadMore(0, 0, $scope.filterData.back_order)
            ...
            $scope.exportExcel = function ()
            {
                ...
                if ($scope.filterData.exportlimit > 0)
                {
                    ...
                    /* disableScreen(1);
                     $scope.loadershow=true; */
                    $http({
                        url: SITEAPP_PATH +"Shipment/exportdispatchExcel",
                        method: "POST",
                        data: $scope.filterData,
                        headers: {'Content-Type': 'application/x-www-form-urlencoded'}
