...
// Properties
    ...
    public function __construct() {
        ...
        $this->fastcoo_order_api = "https://api.fastcoo.net/API/createOrder"; 

        $db_host = 'localhost';
        $db_user = 'root';
        $db_password = "B!M@l1234";
        $db_name = 'fastcoov4';
    ...
    public function allOrders($cust_id) {
        $customers = $this->fetchSallaCustomers($cust_id);
    ...
    private function SallacURL($athentication, $page, $salla_from_date, $order_status) {
        $url ="https://api.salla.dev/admin/v2/orders?expanded=1";
        if(!empty($salla_from_date)){
            $url .= "&from_date=$salla_from_date";
        }
        if(!empty($page)){
            $url .= "&page=$page";
        }
     ...
     * @return type array
     */
    private function fetchSallaCustomers($cust_id) {
        $sql = "select id,secret_key,email,phone,user_Agent,address,seller_id,super_id,zid_active,manager_token,user_Agent as zid_user_Agent,salla_athentication,salla_active,salla_from_date, uniqueid,name,city, order_status,salla_city,company from customer "
                . " where salla_active = 'Y' and status= 'Y' AND uniqueid = '".$cust_id."'";
        $result = $this->db->query($sql);
